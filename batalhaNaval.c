...
#define NAVIO 3
#define AREA 5
// cada tabuleiro(4) representará respectivamente : 
// navios em posiçoes
// navios mais habilidade 1
// navios mais habilidade 2
// navios mais habilidade 3



// utilizar a recursividade para a criação das habilidade
...
void construir_octaedro(int mat[TAM_HABILIDADE][TAM_HABILIDADE]){
    ...
    for(int i=0;i<TAM_HABILIDADE;i++){
        for(int j=0;j<TAM_HABILIDADE;j++){
            if((i-centro<0?-(i-centro):(i-centro)) + (j-centro<0?-(j-centro):(j-centro)) <= centro) //combinando estruturas condicionais
                mat[i][j]=1;
            else 
                mat[i][j]=0;
        }
